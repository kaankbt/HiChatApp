...
import com.kaankubat.hichatapplication.repository.BlockUserRepository;
...
public class UserService implements UserServiceInterface{
	...
	public String save(User user) throws Exception {
		...
		try {
			...
			entity.setActivity(Activity.INVALID_REGISTER);
			entity.setUserName("---");
		...
		entity.setActivity(Activity.REGISTER);
		activityLogEntityRepo.save(entity);
	...
	public Boolean block(String angryName, String blockedName) {
		...
		if(angry.getId() != null && blocked.getId() != null) {
			BlockUserModel blockEntity = new BlockUserModel();
	...
	public Boolean blockControl(String angryName, String blockedName) {
		User angry = userRepo.findByUserName(angryName);
		User blocked = userRepo.findByUserName(blockedName);
		List<BlockUserModel> listOfBlock = blockedUserRepo.findAllByAngryId(angry.getId());
